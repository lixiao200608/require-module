...
define(["jquery"], function(jquery) {
	...
	var flip ={		
        ...
        //填写信息
        formText: function(title,callback){
            flip.flipBody.html("<div class='form-text'><h2>"+title+"</h2><input type='text' value='' class='put'><a class='form-btn' href='javascript:'>提交</a></div>");
            flip.flipFrame.show();
            flip.initial();
            $(".form-btn").off("click").on("click", function() {
                var text = $(".form-text .put").val();
                flip.closeFlip();
                if (callback != null && callback != undefined && callback != "" && callback != "undefined") {
                    callback();
                };
